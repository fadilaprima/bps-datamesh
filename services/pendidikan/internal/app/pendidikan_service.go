...
var SourceMap = map[int]SourceConfig{
	...
	2: {Name: "KEMENDIKBUD", IsWali: true},
...
func (s *PendidikanService) ValidateInternalPendidikan(p *models.RiwayatPendidikan) error {
	...
	if jExists {
		...
		if p.Ijazah == "S1" && jRank <= jenjangRank["SMA"] {
			return fmt.Errorf("memiliki ijazah S1 tetapi jenjang tertinggi <= SMA")
		}
		if p.Ijazah == "SMA" && jRank <= jenjangRank["SMP"] {
			return fmt.Errorf("memiliki ijazah SMA tetapi jenjang tertinggi <= SMP")
		}
		if p.Ijazah == "SMP" && jRank <= jenjangRank["SD"] {
			return fmt.Errorf("memiliki ijazah SMP tetapi jenjang tertinggi <= SD")
		}
		if p.Ijazah == "SD" && jRank <= jenjangRank["TK/Belum sekolah"] {
			return fmt.Errorf("memiliki ijazah SD tetapi jenjang tertinggi <= TK/Belum sekolah")
		}

		// Rule 2: Jenjang vs Kelas Tertinggi
		if p.Jenjang == "SD" && kelasInt > 6 {
			return fmt.Errorf("jenjang SD tetapi kelas yang diduduki > 6")
		}
		if (p.Jenjang == "SMP" || p.Jenjang == "SMA" || strings.HasPrefix(p.Jenjang, "D1") || strings.HasPrefix(p.Jenjang, "D2") || strings.HasPrefix(p.Jenjang, "D3")) && kelasInt > 3 {
			return fmt.Errorf("jenjang %s tetapi kelas yang diduduki > 3", p.Jenjang)
		}
		if (p.Jenjang == "D4" || p.Jenjang == "S1" || p.Jenjang == "S3") && kelasInt > 4 {
			return fmt.Errorf("jenjang %s tetapi kelas yang diduduki > 4", p.Jenjang)
		}
		if p.Jenjang == "S2" && kelasInt > 2 {
			return fmt.Errorf("jenjang S2 tetapi kelas yang diduduki > 2")
		}
	}

	// Rule 3: Partisipasi Sekolah vs Kelas
	if p.Partisipasi == "Tidak/Belum Pernah Sekolah" && kelasInt > 0 {
		return fmt.Errorf("status tidak/belum pernah sekolah tetapi kelas tertinggi > 0")
	}
				...
				if errJson := json.Unmarshal(body, &result); errJson == nil {
					...
					if umur >= 0 {
						...
						if umur < 5 && (p.Partisipasi == "Masih Sekolah" || isJenjangSDKeAtas) {
							return fmt.Errorf("ditolak: umur di bawah 5 tahun tidak wajar untuk partisipasi Masih Sekolah atau menduduki jenjang SD ke atas")
						}
						if umur < 18 && (strings.Contains(p.Ijazah, "S1") || strings.Contains(p.Ijazah, "S2") || strings.Contains(p.Ijazah, "S3")) {
							return fmt.Errorf("ditolak: umur di bawah 18 tahun tidak wajar memiliki ijazah setingkat S1 ke atas")
						}
					}
				}
			}
...
// ==========================================
func (s *PendidikanService) ProcessManualUpdate(nik string, incomingData models.RiwayatPendidikan) (int, error) { 
